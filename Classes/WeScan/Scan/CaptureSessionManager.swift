...
protocol RectangleDetectionDelegateProtocol: NSObjectProtocol {
    ...
    // custom
    func captureSessionManager(_ captureSessionManager: CaptureSessionManager, didCapturedMetadataValue values: [String])
}
...
final class CaptureSessionManager: NSObject, AVCaptureVideoDataOutputSampleBufferDelegate {
    ...
    // 默认的图片质量
    var photoPreset:AVCaptureSession.Preset = AVCaptureSession.Preset.photo
    ...
    var mode: CaptureSessionMode = .single {
        didSet {
            changeMode(mode)
        }
    }
    
    func changeMode(_ mode: CaptureSessionMode) {
        ...
        case .barCode:
            //
            metaDataOutput.metadataObjectTypes = [.ean8, .ean13, .code39, .code93, .code128, .code39Mod43, .pdf417]
            break
        }
    }
    ...
    func captureOutput(_ output: AVCaptureOutput, didOutput sampleBuffer: CMSampleBuffer, from connection: AVCaptureConnection) {
        ...
        guard mode == .single || mode == .batch else {
            return
        }
...
extension CaptureSessionManager: AVCaptureMetadataOutputObjectsDelegate {
    ...
    func metadataOutput(_ output: AVCaptureMetadataOutput, didOutput metadataObjects: [AVMetadataObject], from connection: AVCaptureConnection) {
        ...
        guard mode == .barCode || mode == .qrCode else {
            return
        }
        ...
        delegate?.captureSessionManager(self, didCapturedMetadataValue: values)
        isDetecting = false
